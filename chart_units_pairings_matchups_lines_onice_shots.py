...
def parse_ids(season_id, game_id, images):
    ...
    for team in teams:
        ...
        max_pairings_toi = team_pairings_matchups_lines_df['TOI'].max()
        ...
        team_pairing1_matchups_lines_list = team_pairing1_matchups_lines_df.MATCHUP.unique()
        team_pairing2_matchups_lines_list = team_pairing2_matchups_lines_df.MATCHUP.unique()
        team_pairing3_matchups_lines_list = team_pairing3_matchups_lines_df.MATCHUP.unique()

        if len(team_pairing1_matchups_lines_list) >= 7:
            pairing1_width = .8
        elif len(team_pairing1_matchups_lines_list) == 6:
            pairing1_width = .7
        elif len(team_pairing1_matchups_lines_list) == 5:
            pairing1_width = .6
        elif len(team_pairing1_matchups_lines_list) == 4:
            pairing1_width = .5
        elif len(team_pairing1_matchups_lines_list) == 3:
            pairing1_width = .4
        elif len(team_pairing1_matchups_lines_list) == 2:
            pairing1_width = .3
        elif len(team_pairing1_matchups_lines_list) == 1:
            pairing1_width = .2
            
        if len(team_pairing2_matchups_lines_list) >= 7:
            pairing2_width = .8
        elif len(team_pairing2_matchups_lines_list) == 6:
            pairing2_width = .7
        elif len(team_pairing2_matchups_lines_list) == 5:
            pairing2_width = .6
        elif len(team_pairing2_matchups_lines_list) == 4:
            pairing2_width = .5
        elif len(team_pairing2_matchups_lines_list) == 3:
            pairing2_width = .4
        elif len(team_pairing2_matchups_lines_list) == 2:
            pairing2_width = .3
        elif len(team_pairing2_matchups_lines_list) == 1:
            pairing2_width = .2

        if len(team_pairing3_matchups_lines_list) >= 7:
            pairing3_width = .8
        elif len(team_pairing3_matchups_lines_list) == 6:
            pairing3_width = .7
        elif len(team_pairing3_matchups_lines_list) == 5:
            pairing3_width = .6
        elif len(team_pairing3_matchups_lines_list) == 4:
            pairing3_width = .5
        elif len(team_pairing3_matchups_lines_list) == 3:
            pairing3_width = .4
        elif len(team_pairing3_matchups_lines_list) == 2:
            pairing3_width = .3
        elif len(team_pairing3_matchups_lines_list) == 1:
            pairing3_width = .2
        ...
        # set the plot title
        fig.suptitle(date + ' Pairings vs. Lines On-Ice Shots\n\n')
        ...
        try:
            pairing1_SF_plot = team_pairing1_matchups_lines_df.plot.barh(x='MATCHUP', y='SF', stacked=True, color=pairing1_matchups_lines_toi_color_map_for, width=pairing1_width, legend=None, label='', ax=ax_pairing1_shots);
        except:
            pass
        try:
            pairing1_SA_plot = team_pairing1_matchups_lines_df.plot.barh(x='MATCHUP', y='SA', stacked=True, color=pairing1_matchups_lines_toi_color_map_against, width=pairing1_width, legend=None, label='', ax=ax_pairing1_shots);
        except:
            pass
        try:
            pairing1_GF_marker = team_pairing1_matchups_lines_df.plot(x='GF', y='RANK', marker='D', markersize=5,  markerfacecolor='None', markeredgecolor='white', linewidth=0, alpha=1, legend=None, label='', ax=ax_pairing1_shots);
        except:
            pass
        try:
            pairing1_GA_marker = team_pairing1_matchups_lines_df.plot(x='GA', y='RANK', marker='D', markersize=5,  markerfacecolor='None', markeredgecolor='white', linewidth=0, alpha=1, legend=None, label='', ax=ax_pairing1_shots);
        except:
            pass
        try:
            pairing1_SD_plot = team_pairing1_matchups_lines_df.plot(x='SD', y='RANK', marker='|', markersize=15,  markerfacecolor='None', markeredgecolor='white', linewidth=0, alpha=1, legend=None, label='', ax=ax_pairing1_shots);
        except:
            pass

        try:
            pairing2_SF_plot = team_pairing2_matchups_lines_df.plot.barh(x='MATCHUP', y='SF', stacked=True, color=pairing2_matchups_lines_toi_color_map_for, width=pairing2_width, legend=None, label='', ax=ax_pairing2_shots);
        except:
            pass
        try:
            pairing2_SA_plot = team_pairing2_matchups_lines_df.plot.barh(x='MATCHUP', y='SA', stacked=True, color=pairing2_matchups_lines_toi_color_map_against, width=pairing2_width, legend=None, label='', ax=ax_pairing2_shots);
        except:
            pass
        try:
            pairing2_GF_marker = team_pairing2_matchups_lines_df.plot(x='GF', y='RANK', marker='D', markersize=5,  markerfacecolor='None', markeredgecolor='white', linewidth=0, alpha=1, legend=None, label='', ax=ax_pairing2_shots);
        except:
            pass
        try:
            pairing2_GA_marker = team_pairing2_matchups_lines_df.plot(x='GA', y='RANK', marker='D', markersize=5,  markerfacecolor='None', markeredgecolor='white', linewidth=0, alpha=1, legend=None, label='', ax=ax_pairing2_shots);
        except:
            pass
        try:
            pairing2_SD_plot = team_pairing2_matchups_lines_df.plot(x='SD', y='RANK', marker='|', markersize=15,  markerfacecolor='None', markeredgecolor='white', linewidth=0, alpha=1, legend=None, label='', ax=ax_pairing2_shots);
        except:
            pass

        try:
            pairing3_SF_plot = team_pairing3_matchups_lines_df.plot.barh(x='MATCHUP', y='SF', stacked=True, color=pairing3_matchups_lines_toi_color_map_for, width=pairing3_width, legend=None, label='', ax=ax_pairing3_shots);
        except:
            pass
        try:
            pairing3_SA_plot = team_pairing3_matchups_lines_df.plot.barh(x='MATCHUP', y='SA', stacked=True, color=pairing3_matchups_lines_toi_color_map_against, width=pairing3_width, legend=None, label='', ax=ax_pairing3_shots);
        except:
            pass
        try:
            pairing3_GF_marker = team_pairing3_matchups_lines_df.plot(x='GF', y='RANK', marker='D', markersize=5,  markerfacecolor='None', markeredgecolor='white', linewidth=0, alpha=1, legend=None, label='', ax=ax_pairing3_shots);
        except:
            pass
        try:
            pairing3_GA_marker = team_pairing3_matchups_lines_df.plot(x='GA', y='RANK', marker='D', markersize=5,  markerfacecolor='None', markeredgecolor='white', linewidth=0, alpha=1, legend=None, label='', ax=ax_pairing3_shots);
        except:
            pass
        try:
            pairing3_SD_plot = team_pairing3_matchups_lines_df.plot(x='SD', y='RANK', marker='|', markersize=15,  markerfacecolor='None', markeredgecolor='white', linewidth=0, alpha=1, legend=None, label='', ax=ax_pairing3_shots);
        except:
            pass

        # plot the bars for time on ice
        try:
            toi_pairing1 = team_pairing1_matchups_lines_df.plot.barh(x='MATCHUP', y='TOI', color='white', edgecolor=team_color, width=pairing1_width, legend=None, label='', ax=ax_pairing1_toi);
        except:
            pass

        try:
            toi_pairing2 = team_pairing2_matchups_lines_df.plot.barh(x='MATCHUP', y='TOI', color='white', edgecolor=team_color, width=pairing2_width, legend=None, label='', ax=ax_pairing2_toi);
        except:
            pass

        try:
            toi_pairing3 = team_pairing3_matchups_lines_df.plot.barh(x='MATCHUP', y='TOI', color='white', edgecolor=team_color, width=pairing3_width, legend=None, label='', ax=ax_pairing3_toi);
        except:
            pass
        ...
        SF_max = pairings_matchups_lines_df['SF']
        SF_max = SF_max.max()
        
        SA_max = pairings_matchups_lines_df['SA']
        SA_max = SA_max.max()
        ...
        if team == away:
            plt.savefig(charts_units_pairings_matchups + 'onice_shots_away_pairings_matchups_lines.png', bbox_inches='tight', pad_inches=0.2)
        elif team == home:
            plt.savefig(charts_units_pairings_matchups + 'onice_shots_home_pairings_matchups_lines.png', bbox_inches='tight', pad_inches=0.2)    
